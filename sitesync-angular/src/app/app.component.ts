...
import { AuthService } from '@core/interceptors/services/auth.service';
...
@Component({
    ...
    <div class="shell">
      ...
      <aside class="sidebar">
        ...
        <nav class="nav-section" style="margin-top:14px">
          ...
          @for (item of insightNav; track item.path) {
            <a class="nav-item" [routerLink]="item.path" routerLinkActive="active">
              <span class="nav-icon material-icon">{{ item.icon }}</span>
              <span class="nav-label-text">{{ item.label }}</span>
            </a>
  ...
  <span class="material-icon logout-btn" (click)="logout()" title="Logout">logout</span>
</div>
          </div>
  ...
  styles: [`
    ...
    .sb-footer { margin-top:auto; padding:18px 20px 0; border-top:0.5px solid var(--border); }
    .user-card { display:flex; align-items:center; gap:9px; padding:9px; border-radius:9px; cursor:pointer; }
    ...
    .tb-title { font-family:'Syne',sans-serif; font-weight:700; font-size:17px; letter-spacing:-.5px; color:var(--text); }
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  private sub?: Subscription;
   ...
   userInitials = computed(() =>                // ADD THIS
    this.auth.fullName()
      .split(' ')
      .map(w => w[0] ?? '')
      .join('')
  ...
  mainNav: NavItem[] = [
    ...
    { path: '/sites',    label: 'Sites',     icon: 'home_work',  badge: 8 },
    { path: '/clients',  label: 'Clients',   icon: 'people'       },
    { path: '/labour',   label: 'Labour',    icon: 'engineering'  },
    { path: '/finance',  label: 'Finance',   icon: 'account_balance_wallet' },
  ];
